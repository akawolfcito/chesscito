...
| Cosa | Por qué |
|---|---|
| **Mensaje de rechazo de Privy personalizado** | ✅ **Decidido: se vive sin eso** (founder, 2026-08-10). Hoy el rechazado ve el error genérico de Privy y no se entera de que existe una lista. Se puede arreglar cuando haga falta, pero **sólo por API**: el dashboard NO expone esos campos (el menú `...` de Access control sólo agrega entradas). Requiere el app secret de `Chesscito Prod`, que el `.env.local` **no** tiene — el suyo es de otra app y da 401. Comando listo para correr en la terminal del founder, sin que el secret toque el repo:<br><br>`curl -s -X POST "https://auth.privy.io/api/v1/apps/cms022hew02bz0cjsq4fmd5gu" -u "cms022hew02bz0cjsq4fmd5gu:APP_SECRET" -H "privy-app-id: cms022hew02bz0cjsq4fmd5gu" -H "Content-Type: application/json" --data-raw '{"allowlist_config":{"error_title":"Your journey is almost ready","error_detail":"Chesscito Web is opening gradually to small groups of players. Close this and tap Request access to join the list.","cta_text":"Got it"}}'`<br><br>Campos: `error_title`, `error_detail`, `cta_text`, `cta_link`. ⚠️ `cta_link` es un valor único y la app sirve a learn Y play, así que un link manda a los dos al mismo lado. ⚠️ `allowlist_enabled` **NO** es escribible por API: el POST devuelve 200 y no cambia nada (comprobado). El toggle es sólo del dashboard |
