 ...
 *
 * Literal port of apps/web/src/lib/stats/filters.ts. Its behaviour is already
 * pinned by tests and is correct; it travels unchanged.
 */
...
/**
 * Translate a filter to the RPC argument.
 *
 * ⛔ The string `"all"` must NEVER reach SQL. Every `stats_*` function takes
 * `p_surface`/`p_container` where **null means "no filter"**; a literal `'all'`
 * would match zero rows and every card would read 0. There is a test for this.
 */
export function toRpcArg(value: SurfaceFilter | ContainerFilter): string | null {
