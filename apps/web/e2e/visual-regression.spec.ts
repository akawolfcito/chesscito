...
test.describe("visual regression — Step 3 fixture-driven baselines", () => {
  ...
  // vr13 — Exercises completion popups (Wolfcito polish 2026-06-05). Baselines
  // PieceCompletePrompt + LabyrinthCompleteOverlay + ResultOverlay score
  // variant after their migration to the arena-end-state vocabulary
  // (VictoryPopupShell + panel-bg1 + arena-result-* tokens). Variants cover
  // the CTA cascade (nextPiece / labyrinth / choose / arena fallback), the
  // labyrinth move-quality matrix (perfect / suboptimal / new-best), and the
  // on-chain score-saved celebration including the CeloScan receipt chip.
  test("vr13-piece-complete-next — Start <next piece>", async ({ page }) => {
    await page.goto("/dev/exercises-popups?variant=piece-complete-next", {
      waitUntil: "load",
      timeout: 45_000,
    });
    await page.evaluate(() => document.fonts.ready);
    await settle(page, 800);
    await expect(page).toHaveScreenshot(
      "vr13-piece-complete-next.png",
      FIXTURE_OPTS,
    );
  });

  test("vr13-piece-complete-labyrinth — Try Labyrinth primary + Save secondary", async ({
    page,
  }) => {
    await page.goto("/dev/exercises-popups?variant=piece-complete-labyrinth", {
      waitUntil: "load",
      timeout: 45_000,
    });
    await page.evaluate(() => document.fonts.ready);
    await settle(page, 800);
    await expect(page).toHaveScreenshot(
      "vr13-piece-complete-labyrinth.png",
      FIXTURE_OPTS,
    );
  });

  test("vr13-piece-complete-choose — Choose another piece + Arena link", async ({
    page,
  }) => {
    await page.goto("/dev/exercises-popups?variant=piece-complete-choose", {
      waitUntil: "load",
      timeout: 45_000,
    });
    await page.evaluate(() => document.fonts.ready);
    await settle(page, 800);
    await expect(page).toHaveScreenshot(
      "vr13-piece-complete-choose.png",
      FIXTURE_OPTS,
    );
  });

  test("vr13-piece-complete-arena-fallback — Arena as defensive primary", async ({
    page,
  }) => {
    await page.goto(
      "/dev/exercises-popups?variant=piece-complete-arena-fallback",
      { waitUntil: "load", timeout: 45_000 },
    );
    await page.evaluate(() => document.fonts.ready);
    await settle(page, 800);
    await expect(page).toHaveScreenshot(
      "vr13-piece-complete-arena-fallback.png",
      FIXTURE_OPTS,
    );
  });

  test("vr13-labyrinth-solved-perfect — 3/3 stars optimal path", async ({
    page,
  }) => {
    await page.goto("/dev/exercises-popups?variant=labyrinth-solved-perfect", {
      waitUntil: "load",
      timeout: 45_000,
    });
    await page.evaluate(() => document.fonts.ready);
    await settle(page, 800);
    await expect(page).toHaveScreenshot(
      "vr13-labyrinth-solved-perfect.png",
      FIXTURE_OPTS,
    );
  });

  test("vr13-labyrinth-solved-suboptimal — 1/3 stars over optimal", async ({
    page,
  }) => {
    await page.goto(
      "/dev/exercises-popups?variant=labyrinth-solved-suboptimal",
      { waitUntil: "load", timeout: 45_000 },
    );
    await page.evaluate(() => document.fonts.ready);
    await settle(page, 800);
    await expect(page).toHaveScreenshot(
      "vr13-labyrinth-solved-suboptimal.png",
      FIXTURE_OPTS,
    );
  });

  test("vr13-labyrinth-solved-new-best — celebration pill", async ({ page }) => {
    await page.goto("/dev/exercises-popups?variant=labyrinth-solved-new-best", {
      waitUntil: "load",
      timeout: 45_000,
    });
    await page.evaluate(() => document.fonts.ready);
    await settle(page, 800);
    await expect(page).toHaveScreenshot(
      "vr13-labyrinth-solved-new-best.png",
      FIXTURE_OPTS,
    );
  });
