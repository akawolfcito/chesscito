...
describe("<HubProBadge>", () => {
  it("renders a single 'PRO' tag when inactive", () => {
    render(
      <HubProBadge
        active={false}
    ...
    expect(screen.getByLabelText(/PRO inactive/)).toHaveTextContent(/^PRO$/);
  });

  it("applies the inactive modifier class on the inactive variant", () => {
    render(<HubProBadge active={false} ariaLabel="x" />);
    expect(screen.getByLabelText("x").className).toContain(
      "hub-pro-badge--inactive",
    );
  });

  it("renders 'PRO' + days when active", () => {
    render(
      <HubProBadge
        active
        daysRemaining={7}
  ...
  it("renders as a <button> when onClick is provided", () => {
    render(
      <HubProBadge active={false} ariaLabel="x" onClick={vi.fn()} />,
    );
  ...
  it("fires onClick when tapped (inactive)", () => {
    const onClick = vi.fn();
    render(<HubProBadge active={false} ariaLabel="x" onClick={onClick} />);
    fireEvent.click(screen.getByLabelText("x"));
    expect(onClick).toHaveBeenCalledTimes(1);
  });

  it("omits the days line when active but daysLabel is missing (defensive)", () => {
    render(
      <HubProBadge active daysRemaining={7} ariaLabel="x" />,
    );
    // Tag still renders; days span skipped because there is no formatted
    // label to show. Prevents stray empty `<span>` from inheriting the
    // large-type CSS slot.
    expect(screen.getByLabelText("x")).toHaveTextContent(/^PRO$/);
  });
});
